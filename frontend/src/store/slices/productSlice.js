...
export const fetchAllProducts = createAsyncThunk("/product/fetchAll", async ({ availability = "", price = "0-10000", category = "", ratings = "", search = "", page = 1, }, thunkAPI) => {
  try {
...
export const postReview = createAsyncThunk("/product/post-new/review", async ({ productId, review , user}, thunkAPI) => {
  try {
    const res = await axiosInstance.put(`/product/post-new/review/${productId}`, review);
    toast.success(res.data.message);
    return {review: res.data.review, user};
  } catch (error) {
