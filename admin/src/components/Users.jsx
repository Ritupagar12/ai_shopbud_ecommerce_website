  ...
  return <>
    <main className="p-[10px] pl-[10px] md:pl-[17rem] w-full">
      ...
      <div className="flex-1 md:p-6">
        ...
        <p className="text-sm text-gray-600 mb-6">Manage all your website's user.</p>
        <div className="p-4 sm:p-8 bg-gray-50 min-h-screen">
          <div className={`overflow-x-auto rounded-lg ${loading
            ? "p-10 shadow-none"
                      ...
                      users.map((user, index) => {
                        return (
                          <tr key={index} className="border-t hover:bg-gray-50">
                            ...
                            <td className="px-3 py-4">{new Date(user.created_at).toLocaleDateString()}
                            </td>
                            <td className="px-3 py-4">
                              <button
                                onClick={() => handleDeleteUser(user.id)}
                                className="text-white rounded-md cursor-pointer px-3 py-2 font-semibold bg-red-gradient">Delete</button>
                            </td>
                          </tr>
                        )
                      })
            ...
            !loading && users.length > 0 && (
              <div className="flex justify-center mt-6 gap-4">
                <button
                  ...
                  disabled={page === 1}
                  className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded disabled:opacity-50">Previous</button>
                <span className="px-4 py-2 text-gray-700">Page {page}</span>
                <button
                  onClick={() => setPage((prev) => prev + 1)}
                  disabled={maxPage === page}
                  className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded disabled:opacity-50">Next</button>
              </div>
            )
          }
        </div>
