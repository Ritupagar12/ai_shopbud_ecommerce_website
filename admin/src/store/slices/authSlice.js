...
export const forgotPassword = (email) => async (dispatch) => {
  ...
  try {
    await axiosInstance.post("/auth/password/forgot?frontendUrl=http://localhost:5174", email)
      .then((res) => {
...
export const resetPassword = (newData, token) => async (dispatch) => {
  ...
  try {
    await axiosInstance.put(`/auth/password/reset/${token}`, newData)
      .then((res) => {
...
export const updateAdminProfile = (data) => async (dispatch) => {
  ...
  try {
    await axiosInstance.put("/auth/profile/update", data)
      .then((res) => {
