  ...
  await axiosInstance
    ...
    .then((res) => {
      ...
      if (error.response?.status === 403) {
        toast.info("Read-only demo mode: cannot modify data.");
      } else {
...
export const getDashbaordStats = () => async (dispatch) => {
  dispatch(adminSlice.actions.getStatesRequest());
  await axiosInstance
    .get(`/admin/fetch/dashboard-stats`)
    .then((res) => {
      dispatch(adminSlice.actions.getStatesSuccess(res.data));
    }).catch(error => {
      dispatch(adminSlice.actions.getStatesFailed());
    });
};
